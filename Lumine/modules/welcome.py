...
def new_member(update: Update, context: CallbackContext):
    ...
    for new_mem in new_members:
        ...
        if should_welc:
            ...
            if new_mem.id in BOTKEY:
                deletion(update, context, update.effective_message.reply_photo(
                    TGLOGS, f"{LOGSS}", reply_to_message_id=reply
                ))
                welcome_log = (
            ...
            if new_mem.id == OWNER_ID:
                deletion(update, context, update.effective_message.reply_photo(
                    OWNER, "Oh, the owner just joined, How are you babe?", reply_to_message_id=reply
                ))
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
